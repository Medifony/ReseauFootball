...
class SujetRepository extends ServiceEntityRepository
{
    ...
    public function totSuj($unUser){
        return $this->createQueryBuilder('s')
        ->select('count(s.id)')
        ->where('s.users = \'' . $unUser . '\'')
        ->andWhere('s.championnats IS NOT NULL')
        ->getQuery()
        ->getSingleScalarResult();
    }
        
    
    /*public function sujetsReponsesChamp($champ){
        return $this->createQueryBuilder('s')
                    ->join('s.championnats', 'champ')
                    ->select('s')
                    ->where('champ = \'' . $champ . '\'')
                    ->andWhere('s.clubs IS NULL')
                    ->orderBy('s.sujetDate', 'desc')
                    ->getQuery()
                    ->getResult();
    }
    */
